...
import 'package:custom_pc/domain/price.dart';
...
extension CustomExtension on Custom {
  int calculateTotalPrice() {
    ...
    int totalPrice = 0;
    for (var p in parts!) {
      totalPrice += Price.stringToInt(p.price);
    }
    return totalPrice;
  }
  ...
  Custom updateCompatibilities() {
    ...
    List<PartsCompatibility> comps = [];
    final cpu = parts!.firstWhereOrNull((parts) => parts.category == PartsCategory.cpu);
    final cpuCooler = parts!.firstWhereOrNull((parts) => parts.category == PartsCategory.cpuCooler);
    final memory = parts!.firstWhereOrNull((parts) => parts.category == PartsCategory.memory);
    final motherBoard = parts!.firstWhereOrNull((parts) => parts.category == PartsCategory.motherboard);
    final ssd = parts!.firstWhereOrNull((parts) => parts.category == PartsCategory.ssd);
    ...
    if (ssd != null && motherBoard != null) {
      final compatibility = CompatibilityAnalyzer.analyzeMotherBoardAndSsd(ssd: ssd, motherBoard: motherBoard);
      comps.add(compatibility);
    }
  ...
  String mainPartsImage() {
    ...
    final mainParts = parts!.reduce((a, b) => Price.stringToInt(a.price) > Price.stringToInt(b.price) ? a : b);
