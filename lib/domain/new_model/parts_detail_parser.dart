...
class PartsDetailParserNM {
  ...
  static Future<PcParts> fetch(PcParts parts) async {
    final partsDetailUrl = parts.getDetailUrl();
    final doc = await DocumentRepository.fetchDocument(partsDetailUrl);
    final fullScaleImages = (await _getFullScaleImageCount(partsDetailUrl));
    final partsShops = _getPartsShops(doc);
    final specs = await _getSpecs(partsDetailUrl);

    return parts.copyWith(
      fullScaleImageCount: fullScaleImages,
      shops: partsShops,
      specs: specs,
    );
  }

  static Future<int> _getFullScaleImageCount(String detailUrl) async {
