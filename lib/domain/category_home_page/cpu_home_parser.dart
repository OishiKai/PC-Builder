...
class CpuHomeParser {
  // パース対象のURL
  static const _pageUrl = 'https://kakaku.com/pc/cpu/';

  // 世代のシリーズ名
  static const _cpuSeriesSelector = '#menu > div:nth-child(22) > div > div:nth-child(5) > ul > li';
  ...
  static Future<CpuHome?> fetchAndParse() async {
    _document = await DocumentRepository.fetchDocument(_pageUrl);
    final chips = _parseCpuSeries()!;
    final parts = _parsePopularPats()!;

    // 同メソッド内で _document に代入している為、ここのnullは握りつぶす
    return CpuHome(chips['intel']!, chips['amd']!, parts);
  }

  static Map<String, List<String>>? _parseCpuSeries() {
    if (_document == null) {
      return null;
    }
    final cpuSeriesElement = _document!.querySelectorAll(_cpuSeriesSelector);
    List<String> intelSeries = [];
    List<String> amdSeries = [];
    for (var element in cpuSeriesElement) {
      if (element.text.contains('Core i')) {
        intelSeries.add(element.text.split('(')[0]);
      }
      if (element.text.contains('Ryzen')) {
        amdSeries.add(element.text.split('(')[0]);
      }
    }
    return {'intel': intelSeries, 'amd': amdSeries};
  }
