...
class CpuCoolerHomeParser {
  // パース対象のURL
  static const _pageUrl = 'https://kakaku.com/pc/cpu-cooler/';

  // メーカーリスト
  static const _makerListSelector = '#menu > div:nth-child(17) > div > div > ul > li';
  ...
  // 検索ホーム画面での人気製品の表示数　二列で並べるので偶数にすべし
  static const _popularPartsRequired = 10;

  static Document? _document;
  static Future<CpuCoolerHome> fetchAndParse() async {
    ...
    return CpuCoolerHome(makers, parts);
  }

  static List<String>? _parseMakerList() {
    if (_document == null) {
      return null;
    }
    List<String> makerList = [];
    final makerListElement = _document!.querySelectorAll(_makerListSelector);
    for (var element in makerListElement) {
      makerList.add(element.text.split('(')[0]);
    }
    return makerList;
  }
