...
class PcPartsRepositoryV2 {
  ...
  static Future<int> insertPcParts(PcParts pcParts, String customId) async {
    ...
    // 店情報、スペック情報、画像情報を保存
    await _insertPartsShops(pcParts.shops, pcParts.id);
    await _insertPartsSpecs(pcParts.specs, pcParts.id);
    await _insertFullScaleImages(pcParts.fullScaleImages, pcParts.id);
    return partsId;
  }
  ...
  static Future<List<PcParts>> getAllPcParts() async {
    ...
    for (var map in maps) {
      ...
      final parts = PcParts(
        category: PartsCategory.fromCategoryName(map['category']),
        maker: map['maker'],
  ...
  // CustomIdからPcParts取得
  static Future<List<PcParts>> selectPcPartsById(int id) async {
    final Database db = await DataStoreUseCase.databaseV2;
